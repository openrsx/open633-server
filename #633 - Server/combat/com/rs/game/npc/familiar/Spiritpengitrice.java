...
public class Spiritpengitrice extends Familiar {
	...
	public Spiritpengitrice(Player owner, Pouch pouch, WorldTile tile, int mapAreaNameHash,
	...
	public boolean submitSpecial(Object object) {
		...
		target.ifPlayer(targetSelected -> {
			int level = targetSelected.getSkills().getLevelForXp(Skills.RANGE);
			int drained = 3;
			if (level - drained > 0)
				drained = level;
			targetSelected.getSkills().drainLevel(Skills.RANGE, drained);
		});
