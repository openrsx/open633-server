...
public class DuelArena extends Controller {
				...
				if (hasTarget() && targetConfiguration.hasTarget()) {
					if (controler instanceof DuelArena) {
						...
						if (stage == DuelStage.DONE)
							...
							player.getPackets()
									.sendGameMessage("<col=ff0000>Please check if these settings are correct.");
							...
							oldTarget.getPackets()
									.sendGameMessage("Other player does not have enough space to continue!");
						}
					}
				}
			}
	...
	private void sendRuleFlash(int componentId, int slot) {
		// player.getPackets().sendInterFlashScript(ifFriendly ? 637 : 631, componentId,
		// 11, 2, slot);
		// target.getPackets().sendInterFlashScript(ifFriendly ? 637 : 631, componentId,
		// 11, 2, slot);
	}
	...
	public void endDuel(final Player victor, final Player loser, boolean isDraw) {
		...
		if (rules != null && !rules.hasRewardGiven()) {
			...
			Logger.globalLog(victor.getUsername(), victor.getSession().getIP(),
					new String(" completed the duel with " + loser.getUsername() + " previous items are: "
							+ Arrays.toString(victor.getLastDuelRules().getStake().getShiftedItem()) + " new items are "
							+ Arrays.toString(loser.getLastDuelRules().getStake().getShiftedItem())));
			victor.getLastDuelRules().getStake().clear();
			loser.getLastDuelRules().getStake().clear();
		}
		loser.getPackets().sendGameMessage(isDraw ? "The battle has ended in a draw."
				: "Oh dear, it seems you have lost to " + victor.getDisplayName() + ".");
		victor.getPackets().sendGameMessage(isDraw ? "The battle has ended in a draw."
				: "Congradulations! You easily defeated " + loser.getDisplayName() + ".");
		...
		for (int i = 10; i < 23; i++) {
			if (i == 14) {
				if (player.getEquipment().hasTwoHandedWeapon())
					RSInterfaceDispatcher.sendRemove(target, 3);
			}
			if (player.getLastDuelRules().getRule(i)) {
				slot = i - 10;
				RSInterfaceDispatcher.sendRemove(player, slot);
			}
		}
	}
...
//		player.getDialogueManager().startDialogue("ForfeitDialouge");
	...
	private static final WorldTile[][] POSSIBLE_TILE_CENTRE = {
			{ new WorldTile(3346, 3251, 0), new WorldTile(3376, 3232, 0) }, { new WorldTile(3346, 3214, 0) },
	...
	private static void battleTeleport(Player player, Player target) {
		...
		for (int trycount = 0; trycount < 10; trycount++) {
			...
			if (World.isTileFree(teleTile.getPlane(), teleTile.getX(), teleTile.getY(), player.getSize())
					&& !teleTile.withinDistance(location, 0))
				break;
			teleTile = tile;
		}
