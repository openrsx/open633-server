...
public class LadyOfTheLake extends Dialogue {
	...
	@Override
	public void start() {
		sendNPCDialogue(npcId, NORMAL, "Good day to you, " + (player.getAppearance().isMale() ? "Sir" : "Madam") + ".");
	}

	@Override
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			sendOptionsDialogue("What would you like to say?", "Who are you?", "I seek the sword Excalibur.",
					"Goodbye.");
			stage = 0;
		} else if (stage == 0) {
			...
			int itemId = player.getSkills().hasRequiriments(Skills.COOKING, 80, Skills.FLETCHING, 80, Skills.FISHING,
					76, Skills.FIREMAKING, 75, Skills.DEFENCE, 70, Skills.PRAYER, 70, Skills.WOODCUTTING, 60,
					Skills.MAGIC, 56, Skills.RANGE, 39, Skills.AGILITY, 36, Skills.STRENGTH, 22) ? 14632 : 35;
			player.getInventory().addItemDrop(itemId, 1);
			sendEntityDialogue(IS_ITEM, "", itemId, -1,
					"The Lady of the Lake hands you an " + (itemId == 35 ? "Excalibur" : "Enhanced Excalibur") + ".");
			stage = 14;
		} else if (stage == 14) {
			end();
		}
