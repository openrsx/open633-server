...
public abstract class Dialogue {
	...
	public boolean sendOptionsDialogue(String title, String... options) {
		...
		player.getPackets().sendRunScript(5589, newopts[4], newopts[3], newopts[2], newopts[1], newopts[0],
				options.length);
		return true;
	}
	...
	public boolean sendEntityDialogue(int type, String title, int entityId, int animationId, String... texts) {
		...
		if (type == IS_NPC) {
			...
			player.getInterfaceManager().sendChatBoxInterface(63);
			player.getPackets().sendIComponentText(63, 3, title);
			player.getPackets().sendIComponentText(63, 4, text);
			player.getPackets().sendPlayerOnIComponent(63, 2);
			if (animationId != -1)
